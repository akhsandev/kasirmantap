...
import { db } from '../db';
import { generateBarcode } from '../utils'; // Import fungsi barcode
import { 
    Package, 
    Plus, 
    Search, 
...
// --- KOMPONEN DROPDOWN KATEGORI (Smart Dropdown) ---
const CategoryDropdown = ({ value, onChange, existingCategories }) => {
    ...
    // Cek apakah nilai saat ini adalah custom (tidak ada di daftar kategori)
    useEffect(() => {
        if (value && !existingCategories.includes(value) && value !== '') {
            setIsCustom(true);
        }
    ...
    const handleSelect = (cat) => {
        if (cat === 'OTHER_NEW') {
            setIsCustom(true);
            onChange(''); // Kosongkan agar user bisa mengetik
        } else {
            setIsCustom(false);
            onChange(cat);
        }
    ...
    // Tampilan Mode Input Manual (Custom)
    if (isCustom) {
        return (
            <div className="relative animate-fade-in">
                <input
                    ...
                    className="w-full border-2 border-blue-400 p-2 rounded-lg outline-none focus:ring-2 focus:ring-blue-200 transition-all"
                    placeholder="Ketik Kategori Baru..."
                />
                <button 
                    onClick={() => setIsCustom(false)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-red-500 hover:underline font-bold"
                    type="button"
                >
                    Batal
                </button>
    ...
    // Tampilan Mode Dropdown
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                type="button"
                onClick={() => setIsOpen(!isOpen)}
                className={`w-full flex justify-between items-center border p-2 rounded-lg bg-white transition-all duration-200 ${isOpen ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-300 hover:border-slate-400'}`}
            >
            ...
            <div className={`absolute z-50 mt-2 w-full bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden transition-all duration-300 origin-top ${isOpen ? 'opacity-100 scale-y-100 max-h-60' : 'opacity-0 scale-y-95 max-h-0 pointer-events-none'}`}>
                <div className="overflow-y-auto max-h-60 py-1">
                    {existingCategories.length === 0 && (
                        <div className="p-3 text-xs text-slate-400 text-center italic">Belum ada kategori.</div>
                    )}
                    ...
                    {existingCategories.map((cat, idx) => (
                        <button
                            ...
                            className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center justify-between transition-colors group"
                        >
                    ...
                    <button
                        type="button"
                        onClick={() => handleSelect('OTHER_NEW')}
                        className="w-full text-left px-4 py-3 text-sm text-blue-600 font-bold hover:bg-blue-50 flex items-center gap-2"
                    >
                        <Plus size={14}/> Tambah Kategori Baru / Lainnya...
                    </button>
                </div>
            </div>
        </div>
...
const InventoryView = () => {
    ...
    const [barcodeImg, setBarcodeImg] = useState(null); // Preview Barcode
    ...
    // Efek Samping: Generate Gambar Barcode saat input barcode berubah
    useEffect(() => {
        if (formData.barcode) {
    ...
    const loadProducts = async () => {
        ...
        setProducts(all.reverse()); // Produk terbaru di atas
        
        // Ambil kategori unik
        const cats = [...new Set(all.map(p => p.category).filter(c => c))].sort();
    ...
    const handleSave = async (e) => {
        ...
        if (editingProduct) {
            ...
            const exist = await db.products.where('barcode').equals(payload.barcode).first();
    ...
    const addMultiUnit = () => {
        setFormData({
            ...formData, 
            multi_units: [...formData.multi_units, { name: '', conversion: '', price: '', barcode: '', price_grosir: '' }]
        });
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 shrink-0">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                        <Package className="text-blue-600"/> Stok Barang
                    </h1>
                    <p className="text-slate-500 text-sm">Kelola katalog produk, harga, dan stok.</p>
                </div>
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    >
                        <Plus size={20}/> <span className="hidden md:inline">Produk Baru</span>
                    </button>
                </div>
            </div>
                            ...
                            {filteredProducts.map(p => (
                                <tr key={p.id} className="hover:bg-blue-50/50 transition-colors group">
                                    <td className="px-6 py-4">
                                        ...
                                        {/* Tampilkan Badge Multi Satuan */}
                                        {p.multi_units?.length > 0 && (
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-slate-700">Rp {parseInt(p.price).toLocaleString('id-ID')}</div>
                                        {p.price_grosir > 0 && (
                                            <div className="text-[10px] text-green-600 font-medium">
                                                Grosir: Rp {parseInt(p.price_grosir).toLocaleString()} (Min {p.min_grosir})
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className={`font-bold ${p.stock <= 5 ? 'text-red-600' : 'text-slate-700'}`}>
                                            {p.stock}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleEdit(p)} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200">
                                                <Edit size={16}/>
                                            </button>
                                            <button onClick={() => handleDelete(p.id)} className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200">
                                                <Trash2 size={16}/>
                                            </button>
                                        </div>
                                    </td>
                    ...
                    <div className="bg-white w-full max-w-4xl h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-scale-up">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                                {editingProduct ? <><Edit size={20} className="text-blue-600"/> Edit Produk</> : <><Plus size={20} className="text-blue-600"/> Tambah Produk Baru</>}
                            </h2>
                                        ...
                                        <div>
                                            ...
                                            {barcodeImg && (
                                                <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-lg flex flex-col items-center">
                                                    ...
                                                    <button type="button" onClick={handleDownloadBarcode} className="mt-2 text-[10px] text-blue-600 hover:underline flex items-center gap-1">
                                                        <Download size={10}/> Download Label
                                                    </button>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 mb-1">Modal (HPP)</label>
                                            <input type="number" required value={formData.cost_price} onChange={e => setFormData({...formData, cost_price: e.target.value})} className="w-full border p-2 rounded-lg" placeholder="0" />
                                        </div>
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-blue-600 mb-1">Harga Jual (Umum)</label>
                                            <input type="number" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} className="w-full border-2 border-blue-100 p-2 rounded-lg font-bold text-slate-800 focus:border-blue-500" placeholder="0" />
                                        </div>
                                        <div className="col-span-2 md:col-span-1">
                                            <label className="block text-xs font-bold text-slate-500 mb-1">Stok Awal</label>
                                            <input type="number" value={formData.stock} onChange={e => setFormData({...formData, stock: e.target.value})} className="w-full border p-2 rounded-lg bg-yellow-50" placeholder="0" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-dashed grid grid-cols-2 gap-4 bg-green-50/50 p-3 rounded-lg">
                                        <div>
                                            <label className="block text-xs font-bold text-green-700 mb-1">Harga Grosir (Member)</label>
                                            <input type="number" value={formData.price_grosir} onChange={e => setFormData({...formData, price_grosir: e.target.value})} className="w-full border p-2 rounded-lg border-green-200" placeholder="Opsional" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-green-700 mb-1">Min. Beli (Qty)</label>
                                            <input type="number" value={formData.min_grosir} onChange={e => setFormData({...formData, min_grosir: e.target.value})} className="w-full border p-2 rounded-lg border-green-200" placeholder="Contoh: 10" />
                                        </div>
                                    </div>
                                </div>

                                {/* 3. MULTI SATUAN (DUS/PAK) */}
                                <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                                    <div className="flex justify-between items-center mb-4 border-b pb-2">
                                        <h3 className="font-bold text-slate-700 text-sm uppercase tracking-wide">Multi Satuan (Opsional)</h3>
                                        <button type="button" onClick={addMultiUnit} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-lg font-bold hover:bg-indigo-100 border border-indigo-200 flex items-center gap-1">
                                    ...
                                    <div className="space-y-3">
                                        {formData.multi_units.map((unit, idx) => (
                                            <div key={idx} className="flex flex-col md:flex-row gap-2 items-start md:items-center bg-slate-50 p-3 rounded-lg border border-slate-200 relative group">
                                                ...
                                                <button type="button" onClick={() => removeMultiUnit(idx)} className="text-slate-400 hover:text-red-500 p-1">
                                                    <Trash2 size={16}/>
                                                </button>
                                            </div>
                                        ))}
                        ...
                        <div className="p-4 bg-white border-t border-slate-200 flex justify-end gap-3 shrink-0">
                            ...
                            <button form="productForm" type="submit" className="px-6 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 flex items-center gap-2">
